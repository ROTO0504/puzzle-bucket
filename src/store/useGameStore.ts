...
const INITIAL_ROTATION: OrientationRotation = { yaw: 0, pitch: 0 };
...
const createGhost = (
  ...
  const clamped = clampToBounds(snapped, spec, rotationClone, BASKET_DIMENSIONS);
  const isValid = !gridCollision(
    { spec, position: clamped, rotation: rotationClone },
...
const evaluateScore = (placed: PlacedInstance[]): ScoreBreakdown => {
  ...
  const stability = computeStabilityScore(0);
...
export const useGameStore = create<GameStoreState>((set, get) => ({
  phase: "placing" satisfies GamePhase,
  ...
  actions: {
    setGhostPosition: (position) => {
      ...
      const clamped = clampToBounds(snapped, state.ghostInstance.spec, state.ghostInstance.rotation, BASKET_DIMENSIONS);
      const isValid = !gridCollision(
        { spec: state.ghostInstance.spec, position: clamped, rotation: state.ghostInstance.rotation },
    ...
    rotateGhostYaw: (direction = 1) => {
      ...
      const clamped = clampToBounds(ghost.position, ghost.spec, nextRotation, BASKET_DIMENSIONS);
      const isValid = !gridCollision(
        { spec: ghost.spec, position: clamped, rotation: nextRotation },
        toCandidatePlacements(state.placedInstances),
        BASKET_DIMENSIONS,
      );
      set({
